...
export default function DraggableHeart({ onHeartDrop, products }: DraggableHeartProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const mouseMoveHandler = (e: MouseEvent) => {
      ...
      if (productCard) {
        ...
        console.log('All products:', products.map(p => ({ id: p.id, isLiked: p.isLiked })));
        if (product) {
  ...
  return (
    <>
      ...
      >
        <div className={`w-full h-full flex items-center justify-center bg-white rounded-full shadow-lg border-2 ${
      ...
      {/* 드래그 중일 때 가이드 텍스트 */}
      {isDragging && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-lg z-50 ${
          ...
          <div className="text-xs mt-1">canDrop: {canDrop.toString()}</div>
        </div>
      )}
